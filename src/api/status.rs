...
///
/// `STABLE` (reads config + catalog, no writes) and `parallel_safe`. Safe
/// to `GRANT EXECUTE ... TO PUBLIC`: the document reports
/// `provider_configured` as a boolean and never returns the api_key.
